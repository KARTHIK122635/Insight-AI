...
export const Sidebar: React.FC<SidebarProps> = ({
  ...
  // 1. Power BI Desktop Style Navigation Rail for Workspace Home
  if (activeTab === 'workspace_home') {
    return (
      <aside className="w-16 bg-[#161820] border-r border-[#242834] flex flex-col justify-between items-center py-3 z-30 flex-shrink-0 select-none">
        {/* Top Rail Navigation */}
        <div className="w-full flex flex-col items-center space-y-1">
          {/* Home Button (Active with Left Indicator) */}
          <button
            onClick={() => onSelectTab('workspace_home')}
            className="w-full relative flex flex-col items-center py-2.5 text-white group focus:outline-none bg-slate-800/50"
            title="Home"
          >
            <span className="absolute left-0 top-1 bottom-1 w-1 bg-amber-400 rounded-r shadow-[0_0_8px_rgba(251,191,36,0.6)]" />
            <Icon name="home" className="w-5 h-5 text-amber-400 mb-1 group-hover:scale-110 transition-transform" />
            <span className="text-[10px] font-semibold text-slate-200">Home</span>
          </button>

          {/* Open Button */}
          <button
            onClick={onOpenUpload}
            className="w-full flex flex-col items-center py-2.5 text-slate-400 hover:text-white transition-colors group focus:outline-none hover:bg-slate-800/30"
            title="Open report or dataset file"
          >
            <Icon name="folder" className="w-5 h-5 text-slate-400 group-hover:text-amber-400 mb-1 group-hover:scale-110 transition-transform" />
            <span className="text-[10px] font-medium text-slate-300">Open</span>
          </button>

          {/* Divider */}
          <div className="w-8 border-b border-[#2d3240] my-2" />

          {/* MongoDB Database Button */}
          <button
            onClick={onOpenMongoModal}
            className="w-full flex flex-col items-center py-2 text-slate-400 hover:text-emerald-400 transition-colors group focus:outline-none hover:bg-slate-800/30"
            title="Connect MongoDB Database"
          >
            <span className="text-lg leading-none mb-1 group-hover:scale-110 transition-transform">🍃</span>
            <span className="text-[9px] font-medium text-slate-300">MongoDB</span>
          </button>

          {/* Studio Analytics Access Button (when dataset exists) */}
          {activeDatasetId && (
            <button
              onClick={() => onSelectTab('dashboard')}
              className="w-full flex flex-col items-center py-2 text-slate-400 hover:text-indigo-400 transition-colors group focus:outline-none hover:bg-slate-800/30"
              title="Return to Analytics Studio"
            >
              <Icon name="layout-dashboard" className="w-5 h-5 text-indigo-400 mb-1 group-hover:scale-110 transition-transform" />
              <span className="text-[9px] font-medium text-indigo-300">Studio</span>
            </button>
          )}
        </div>
        ...
        <div className="w-full flex flex-col items-center space-y-2">
          ...
          <button
            onClick={onOpenMongoModal}
            className="w-full flex flex-col items-center py-2 text-slate-400 hover:text-white transition-colors group focus:outline-none hover:bg-slate-800/30"
            title="Options and settings"
          >
            <Icon name="settings" className="w-5 h-5 text-slate-400 group-hover:text-slate-200 mb-1 group-hover:scale-110 transition-transform" />
            <span className="text-[9px] font-medium text-slate-400 text-center leading-tight">Settings</span>
          </button>

          {/* About */}
          <button
            onClick={onOpenAbout || (() => {
              alert("InsightAI Intelligence Platform v2.0\nDual Engine: DuckDB Vector OLAP + MongoDB Document Store\nZero-latency In-Memory Cache & Spatial Gestures");
            })}
            className="w-full flex flex-col items-center py-2 text-slate-400 hover:text-white transition-colors group focus:outline-none hover:bg-slate-800/30"
            title="About InsightAI"
          >
            <Icon name="info" className="w-5 h-5 text-slate-400 group-hover:text-slate-200 mb-1 group-hover:scale-110 transition-transform" />
            <span className="text-[9px] font-medium text-slate-400">About</span>
          </button>
        </div>
      </aside>
    );
  ...
  // 2. Full Analytics Studio Sidebar (when exploring reports and analytical labs)
  return (
    <aside
